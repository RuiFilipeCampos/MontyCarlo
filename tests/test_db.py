...
class AreDBPresent(ut.TestCase):
    ...
    def test_electron_elastic(self):
        ...
        self.assertTrue(work_dir.exists(), msg = f"`{str(work_dir)}` directory does not exist.")
        self.assertTrue(work_dir.is_dir(), msg = f"`{str(work_dir)}` is not a directory.")
