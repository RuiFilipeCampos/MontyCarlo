...
cdef class Positron(Particle):
    ...
    @staticmethod
    cdef Positron _newISOTROPIC(double E,
              double x, double y, double z,
              Volume current_region,
...
#        self.throwAZIMUTH()
   ...
   #     if# self.z == INFINITY or self.z == -INFINITY: print("------------------------------------------- INTIALIZED LIKE THIS")
    ...
    cdef inline void update_imfp_cumul(Positron self):
        ...
        self.IMFP_CUMUL.C0 = self.anih.imfpA[i] + self.anih.imfpB[i]*self.E
        self.IMFP_CUMUL.C1 = self.IMFP_CUMUL.C0 + self.inelastic.imfpA[i] + self.inelastic.imfpB[i]*self.E
        ...
        #self.positron.IMFP(self.E)
        self.s_max = 4/self.IMFP_CUMUL.C3
        cdef double Emin = self.E - self.positron.find_wmax(self.s_max, self.E)
    ...
    cdef inline void update_imfp(Positron self) :
        ...
        if Emin is 0:
            j = self.find_index()
            self.imfp_max = self.positron.imfpA[j] + self.positron.imfpB[j] * self.E
        ...
        self.s_max = self.positron.imfpA[j] + self.positron.imfpB[j] * self.E
    ...
    cdef inline void sample_w(self):
        ...
        self.avgW = self.s * ( self.positron.softSPA[i]     + self.positron.softSPB[i]*self.E)
        self.varW = self.s * ( self.positron.softSTRAGGA[i] + self.positron.softSTRAGGB[i]*self.E)

        cdef double avgW2 = pow(self.avgW, 2)
        
        if avgW2 > 9*self.varW:

            self.w = sqrt(self.varW)*self.current_material.positron.gauss._sample() + self.avgW #CONFIRM
            return
    ...
    cdef inline void _inelastic(self):
        ...
        if Uk < self.E < Wk:
            ...
            gamma = (self.E/ELECTRON_REST_MASS + 1)**2
            
            a = (self.E/(self.E + ELECTRON_REST_MASS))**2
            gp1 = (gamma + 1)**2
            ...
            while 1:
                r = kc / ( 1 - self.genPTR.get_next_float()*(1 - kc) )
                if self.genPTR.get_next_float() < r*r*(1/(r*r)  - b1/r + b2 - b3*r + b4*r*r): break
    ...
    cdef void _anihilation(self):
        cdef double gamma = self.E/0.511e6 + 1
        cdef double f_min = 1/(gamma + 1 + (gamma**2 - 1)**.5)
        cdef double v, r
        while 1:
            v = f_min * ((1-f_min)/f_min)**self.genPTR.get_next_float()
            if self.genPTR.get_next_float()*g(f_min, gamma) <= g(v, gamma):
                break
        ...
        self.throwAZIMUTH()
        cdef Photon p = Photon._new(Eplus,
                                    self.x, self.y, self.z,
                                    self.eyx, self.eyy, self.eyz,
                                    self.ezx, self.ezy, self.ezz,
                                    self.current_region)
