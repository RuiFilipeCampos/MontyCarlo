...
cdef class Particle:
    ...
    cdef void normalize(self):
        ...
        x = 1 - (self.state.dire.x*self.state.dire.x + self.state.dire.y *
                 self.state.dire.y + self.state.dire.z*self.state.dire.z)
    ...
    cdef void rotateTHETA(self, long double cos):
        ...
        cdef long double cos2sq = .5*(1 + cos)
        cdef long double cos2 = sqrt(cos2sq)

        cdef long double sin2sq = 1 - cos2sq
        cdef long double sin2 = sqrt(sin2sq)
        cdef long double sin2cos2 = 2*sin2*cos2

        cdef long double eyx2 = self.state.axis.x**2
        cdef long double eyy2 = self.state.axis.y**2
        cdef long double eyz2 = self.state.axis.z**2

        cdef long double ezx = self.state.dire.x
        cdef long double ezy = self.state.dire.y
        cdef long double ezz = self.state.dire.z
        ...
        self.state.dire.x = \
            ...
            sin2sq * (eyx2 * ezx
                      + 2*self.state.axis.x *
                      (self.state.axis.y*ezy + self.state.axis.z*ezz)
                      - ezx * (eyy2 + eyz2))         \
...
# ('norm:', 1.0)
        ...
        cos2sq = 1 - (self.state.dire.x**2 + self.state.dire.y **
                      2 + self.state.dire.z**2)
