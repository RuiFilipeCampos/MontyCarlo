...
const Directory = ({ folder_id, project_id }) => {
  const [state, setState] = useState({
    name: '',
    is_open: false,
    directories: [],
  ...
  const onClickNewFolder = (name) => {
    post_folder(name, project_id, folder_id).then(
      (e)=>setforceUpdate(forceUpdate+1)
    )
  ...
  const onClickDeleteFolder = (name) => {
    delete_folder(project_id, folder_id).then((e)=>setforceUpdate(forceUpdate+1))
    folderDisclosure.onClose()
    
  }

  if (state.is_open) return <VStack spacing={.01}
    ...
    <HStack px={2} spacing={.001} align="right">
      <Box w="1px" h="full" bg={highlight}><></></Box>
      <VStack align="left" w="full" spacing={1}>
        <Spacer />
        {
          state.directories.map(
            (id) => <Directory project_id={3} folder_id={id}/>
          )
        }
        {
...
const IndexPage = () => {
  ...
  return <Layout title="Home | Next.js + TypeScript + Electron Example">
    <Flex  minW="200px" h="inherit" w="15%">
